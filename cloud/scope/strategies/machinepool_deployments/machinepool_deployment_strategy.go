...
// Package machinepool implements the machine pool deployment strategies for GCPMachinePool.
package machinepool
...
type (
	...
	DeleteSelector interface {
		SelectMachinesToDelete(ctx context.Context, desiredReplicas int32, machinesByProviderID map[string]infrav1exp.GCPMachinePoolMachine) ([]infrav1exp.GCPMachinePoolMachine, error)
	}
	...
	TypedDeleteSelector interface {
		DeleteSelector
		Type() infrav1exp.GCPMachinePoolDeploymentStrategyType
	}
...
// NewMachinePoolDeploymentStrategy constructs a strategy implementation described in the GCPMachinePoolDeploymentStrategy
// specification.
func NewMachinePoolDeploymentStrategy(strategy infrav1exp.GCPMachinePoolDeploymentStrategy) TypedDeleteSelector {
	switch strategy.Type {
	case infrav1exp.RollingUpdateGCPMachinePoolDeploymentStrategyType:
...
// Type is the GCPMachinePoolDeploymentStrategyType for the strategy.
func (rollingUpdateStrategy *rollingUpdateStrategy) Type() infrav1exp.GCPMachinePoolDeploymentStrategyType {
...
// the DeletePolicy.
func (rollingUpdateStrategy rollingUpdateStrategy) SelectMachinesToDelete(ctx context.Context, desiredReplicaCount int32, machinesByProviderID map[string]infrav1exp.GCPMachinePoolMachine) ([]infrav1exp.GCPMachinePoolMachine, error) {
	...
	var (
		order = func() func(machines []infrav1exp.GCPMachinePoolMachine) []infrav1exp.GCPMachinePoolMachine {
	...
	if len(deletingMachines) > 0 {
		log.Info("failed or deleting machines", "desiredReplicaCount", desiredReplicaCount, "maxUnavailable", maxUnavailable, "deletingMachines", getProviderIDs(deletingMachines))
		return deletingMachines, nil
	}
	...
	if len(readyMachines) < int(desiredReplicaCount) {
		...
		return []infrav1exp.GCPMachinePoolMachine{}, nil
	}

	// we have too many machines, let's choose the oldest to remove
	if overProvisionCount > 0 {
		var toDelete []infrav1exp.GCPMachinePoolMachine
	...
	if len(machinesWithoutLatestModel) == 0 {
		log.Info("nothing more to do since all the GCPMachinePoolMachine(s) are the latest model and not over-provisioned")
		return []infrav1exp.GCPMachinePoolMachine{}, nil
	}
...
func getReadyMachines(machinesByProviderID map[string]infrav1exp.GCPMachinePoolMachine) []infrav1exp.GCPMachinePoolMachine {
	var readyMachines []infrav1exp.GCPMachinePoolMachine
...
func getMachinesWithoutLatestModel(machinesByProviderID map[string]infrav1exp.GCPMachinePoolMachine) []infrav1exp.GCPMachinePoolMachine {
	var machinesWithLatestModel []infrav1exp.GCPMachinePoolMachine
	for _, v := range machinesByProviderID {
		if !v.Status.LatestModelApplied {
...
// but whose corresponding GCPMachinePoolMachine resource has not yet been marked for deletion.
...
func orderByNewest(machines []infrav1exp.GCPMachinePoolMachine) []infrav1exp.GCPMachinePoolMachine {
...
func orderByOldest(machines []infrav1exp.GCPMachinePoolMachine) []infrav1exp.GCPMachinePoolMachine {
	sort.Slice(machines, func(i, j int) bool {
...
func orderRandom(machines []infrav1exp.GCPMachinePoolMachine) []infrav1exp.GCPMachinePoolMachine {
...
func getProviderIDs(machines []infrav1exp.GCPMachinePoolMachine) []string {
