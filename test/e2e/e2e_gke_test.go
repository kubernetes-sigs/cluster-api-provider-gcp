...
var _ = Describe("GKE workload cluster creation", func() {
	...
	Context("Creating a GKE cluster with a maintenance policy", func() {
		It("Should create a cluster with MaintenancePolicy set", func() {
			clusterName := fmt.Sprintf("%s-maintenance", clusterNamePrefix)
			By("Initializes with a daily maintenance window")
			...
			ApplyManagedClusterTemplateAndWait(ctx, ApplyManagedClusterTemplateAndWaitInput{
				...
				ConfigCluster: clusterctl.ConfigClusterInput{
					...
					KubeconfigPath:           bootstrapClusterProxy.GetKubeconfigPath(),
					InfrastructureProvider:   clusterctl.DefaultInfrastructureProvider,
					Flavor:                   "ci-gke-maintenance-policy",
					Namespace:                namespace.Name,
					ClusterName:              clusterName,
					KubernetesVersion:        e2eConfig.MustGetVariable(KubernetesVersionGKE),
					ControlPlaneMachineCount: ptr.To[int64](1),
