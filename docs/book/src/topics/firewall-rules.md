...
CAPG creates firewall rules during cluster provisioning and deletes them when the cluster is deleted. On every reconcile loop it also compares each rule in the spec against the rule that exists in GCP and **updates** the rule in place when they differ, so changes to the spec are rolled out automatically. Because the rule in GCP is replaced with the spec, any out-of-band change made directly in GCP is reverted on the next reconcile.

Every rule CAPG creates is recorded in `status.network.firewallRules`. A recorded rule that the spec no longer asks for is **deleted** on the next reconcile, so removing a rule from the spec removes it from GCP, and renaming one deletes the rule that carried the old name. Rules CAPG did not create are never recorded and never deleted.
...
4. Compare `spec.network.firewall.firewallRules` with `status.network.firewallRules`, which lists the rules CAPG has created
